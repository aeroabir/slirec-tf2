...
def data_processing(input_file, output_file, negative_sampling_value=1):
    ...
    for line in f_input:
        ...
        if user == last_user:
            if (
                i < user_count[user] - 1 - negative_sampling_value
            ):  # 1 + negative samples
...
if __name__ == "__main__":
    ...
    # meta_readfile = "meta_Electronics.json"
    # reviews_readfile = "reviews_Electronics.json"
    ...
    negative_sampling(
        reviews_writefile, meta_writefile, ns_file, negative_sampling_value=1
    )
